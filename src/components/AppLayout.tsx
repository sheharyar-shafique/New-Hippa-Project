...
const navItems = [
  ...
  { to: '/app/new', label: 'New consultation', icon: Mic },
  { to: '/app/labs', label: 'Lab analysis', icon: FlaskConical },
...
export default function AppLayout() {
  ...
  const doctor = { name: 'Dr. Alex Reyes', role: 'Internal Medicine' };
    ...
    <div className="min-h-screen bg-ink-50 flex">
      ...
      <aside className="hidden md:flex w-64 shrink-0 flex-col border-r border-ink-200 bg-white">
        ...
        <nav className="flex-1 px-3 py-4 space-y-1">
          <p className="section-title px-2 pb-2">Workspace</p>
          {navItems.map((item) => (
            <NavLink
              key={item.to}
              to={item.to}
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <header className="sticky top-0 z-30 bg-white/90 backdrop-blur border-b border-ink-200">
          <div className="flex items-center gap-3 px-4 md:px-8 h-16">
            ...
            <button className="btn-ghost p-2.5 rounded-full" aria-label="Help">
              <HelpCircle className="w-5 h-5" />
            </button>
            <button className="btn-ghost p-2.5 rounded-full relative" aria-label="Notifications">
